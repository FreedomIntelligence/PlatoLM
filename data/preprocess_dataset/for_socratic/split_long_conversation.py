...
Usage: 
python3 -m split_long_conversation_revised \
    --in merged_html_en_mine.json 
    --out merged_html_en_mine_spilt_mine.json 
    --model-name-or-path /mntcephfs/data/med/zhihong/workspace/LLMZoo/llama_hf_7b
"""
import argparse
import json
    ...
    for sample in tqdm.tqdm(content): 
        ...
        for idx, l in enumerate(tokenized_lens):
            if num_tokens + l > max_length:
                ...
                # specify the long conversations started from gpt
                if sample["conversations"][idx]["from"] == 'human':
                    start_idx = idx - 1
                    num_tokens = l + tokenized_lens[idx-1]
                else: 
                    start_idx = idx
                    num_tokens = l
            else:
