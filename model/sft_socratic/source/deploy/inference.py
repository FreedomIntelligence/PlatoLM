...
def chat_loop(
    ...
    model, tokenizer = load_model(
        model_path, device, num_gpus, max_gpu_memory, debug
    )

    if conv_template is None:
        conv = get_default_conv_template().copy()
    else:
        train_conv = get_default_conv_template()
        train_conv.system = conv_template
    ...
    while True:
        ...
        chatio.prompt_for_output(conv.roles[0])
